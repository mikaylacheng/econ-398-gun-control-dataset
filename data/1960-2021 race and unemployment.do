...
gen nativeamerican=.
replace nativeamerican=1 if race==3
replace nativeamerican=0 if race!=3

gen asian=.
replace asian=1 if race>3 & race <7
replace asian=0 if race<=3 & race>=7

gen other=.
replace other=1 if race>=7
replace other=0 if race<7

*create unemployed binary variable so you can take the mean later

gen unemployed=.
replace unemployed=1 if empstat==2
replace unemployed=0 if empstat!=2





*once above done, collapse 

// collapse (mean) unemployed (sum) white black asian nativeamerican other, by(state year)
// gen p_white = white/(white+black+asian+nativeamerican+other)

*now create propotions

gen p_white = white/(white+black+asian+nativeamerican+other)
gen p_black = black/(white+black+asian+nativeamerican+other)
gen p_nativeamerican = nativeamerican/(white+black+asian+nativeamerican+other)
gen p_asian = asian/(white+black+asian+nativeamerican+other)
gen p_other = other/(white+black+asian+nativeamerican+other)

gen p_nonwhite= (black+asian+nativeamerican+other)/(white+black+asian+nativeamerican+other)
